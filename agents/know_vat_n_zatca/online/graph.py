...
agents/chatbot/langgraph_chatbot.py. Read-only against Qdrant + SQLite —
this pipeline never writes to the knowledge base.

Does NOT touch agents/chatbot/langgraph_chatbot.py — detect_language() is
imported and reused, not copied or modified, per
docs/rag_agent_handoff.pdf §7.2.
"""
...
from agents.chatbot.memory.persistence import DB_HOST, DB_NAME, DB_PASSWORD, DB_PORT, DB_USER
...
class KnowVatZatcaAgent:
    ...
    def _generate_answer_node(self, state: KnowVatZatcaState) -> dict:
        ...
        try:
            ...
            content = body.get("content") or body.get("reply") or body.get("message") or ""
