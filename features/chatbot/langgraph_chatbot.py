    ...
    def _generate_reply(self, state: ChatState) -> dict:
        ...
        if self.model_url:
            try:
                ...
                if content:
                    content = strip_latex_math(str(content))
                    return {"messages": [AIMessage(content=content)], "reply": content}
...
# frontend/documentation.html is the single source of truth for what the
# app's features do (see documentation.html itself) -- rather than keeping
# a second, hand-written summary here that would drift out of sync, the
# assistant reads the same English content straight from that file and
# points users at its numbered sections. Requires the service to have
# frontend/ available at build time (it does -- the Dockerfile COPYs the
# whole repo) and to be rebuilt after a documentation.html edit, same as
# any other change to this module.
_DOCUMENTATION_PATH = Path(__file__).resolve().parents[2] / "frontend" / "documentation.html"
_DOC_BLOCK_TAGS = {"p", "li", "tr", "h2", "h3", "h4"}


class _DocTextExtractor(HTMLParser):
    """Strips a documentation.html fragment down to plain text. Section
    numbers are already baked into each heading's own text ("4.2 Prepare a
    period VAT return"), so they survive here for free -- letting the
    model cite a specific section instead of paraphrasing the whole page.
    Skips the small "switch language" link, which is page UI, not content."""
    ...
    def handle_starttag(self, tag, attrs):
        if self._skip_depth or dict(attrs).get("class") == "mz-lang-switch":
            self._skip_depth += 1
            return
...
def _load_documentation_text() -> str:
    try:
        raw = _DOCUMENTATION_PATH.read_text(encoding="utf-8")
        start = raw.index(">", raw.index('id="content-en"')) + 1
        end = raw.rindex("<div", 0, raw.index('id="content-ar"'))
    except (OSError, ValueError):
        # Missing file (not yet rebuilt into this image) or unexpected
        # markup -- the assistant still works, just without doc citations.
        return ""
    extractor = _DocTextExtractor()
    extractor.feed(raw[start:end])
...
# Read once at import time, not per request -- the file is effectively
# static, and redeploying after a documentation.html edit already re-runs
# this module fresh.
FEATURE_DOCUMENTATION = _load_documentation_text()
...
def build_system_prompt(language: str = "en", profile: Optional[dict] = None) -> str:
    ...
    doc_block = ""
    if FEATURE_DOCUMENTATION:
        doc_block = f"""

Feature documentation (Mizan.ai's own Documentation page, numbered sections — use this to answer what a feature does or how to use it):
{FEATURE_DOCUMENTATION}

When you answer using the documentation above, mention the specific section number and title so the user can read more themselves (e.g. "see section 4.2 Prepare a period VAT return in the Documentation page"; in Arabic: "راجع القسم 4.2 في صفحة التوثيق"). Only cite a section when you actually used it to answer — not for small talk or unrelated replies."""
...
- If the user asks about Mizan.ai, explain what it is and its main features, using the feature documentation below.
